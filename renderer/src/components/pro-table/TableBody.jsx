...
import {
  GRID_STYLE, CELL, CELL_INNER,
  Avatar, StatusBadge, StockStatusBadge,
  EDITABLE_COLS, STOCK_STATUS_OPTIONS, STATUS_OPTIONS,
} from './tableUtils';
...
function TextCell({ value, onCommit, onNavigate, isDirty }) {
  ...
  return (
    <div
      onDoubleClick={start}
      onClick={e => e.stopPropagation()}
      className={`w-full h-full flex items-center px-3 cursor-text rounded group ${
        isDirty ? 'bg-yellow-50 dark:bg-yellow-400/10' : 'hover:bg-gray-50 dark:hover:bg-white/5'
      }`}
...
function NumberCell({ value, onCommit, onNavigate, isDirty, prefix, isInt }) {
  ...
  return (
    <div
      onDoubleClick={start}
      onClick={e => e.stopPropagation()}
      className={`w-full h-full flex items-center px-3 cursor-text rounded ${
        isDirty ? 'bg-yellow-50 dark:bg-yellow-400/10' : 'hover:bg-gray-50 dark:hover:bg-white/5'
      }`}
      title="Double-click to edit"
    >
      <span className={`text-[12px] font-mono ${
        isDirty ? 'text-yellow-700 dark:text-yellow-300 font-medium'
        : value > 0 ? 'text-[#1a1a1a] dark:text-white/70' : 'text-[#ccc] dark:text-white/15'
      }`}>
        {display}
      </span>
    </div>
...
function SelectCell({ value, options, onCommit, onNavigate, isDirty, renderValue }) {
  ...
  return (
    <div
      onDoubleClick={start}
      onClick={e => e.stopPropagation()}
      className={`w-full h-full flex items-center px-3 cursor-pointer rounded ${
        isDirty ? 'bg-yellow-50 dark:bg-yellow-400/10' : 'hover:bg-gray-50 dark:hover:bg-white/5'
      }`}
      title="Double-click to edit"
    >
...
const ProductRow = memo(({ product, isSelected, onRowClick, onToggleOne, dirtyFields, onCellCommit, onNavigate }) => {
  ...
  return (
    <div
      style={{ ...GRID_STYLE, height: ROW_HEIGHT }}
      className={`border-b border-[#f5f3f0] dark:border-white/5 transition-colors duration-100 ${
        isSelected ? 'bg-yellow-50/60 dark:bg-yellow-400/5' : ''
      }`}
    >
      {/* Checkbox */}
      <div className={CELL} onClick={e => { e.stopPropagation(); onToggleOne(rowKey); }}>
        <input
          type="checkbox"
          checked={isSelected}
          onChange={() => onToggleOne(rowKey)}
          className="w-3.5 h-3.5 rounded border-[#ccc] dark:border-white/20 cursor-pointer accent-[#1a1a1a] dark:accent-white"
        />
      </div>

      {/* Name — double-click edits inline, single click opens modal */}
      <div className="flex items-center gap-2.5 min-w-0 pr-2">
        <div
          className="flex-shrink-0 pl-3 cursor-pointer"
      ...
      <div className="flex items-center min-w-0">
        <NumberCell
          value={merged.regular_price}
          isDirty={isDirty('regular_price')}
          prefix="$"
      ...
      <div className="flex items-center min-w-0">
        <NumberCell
          value={merged.sale_price}
          isDirty={isDirty('sale_price')}
          prefix="$"
      ...
      <div className="flex items-center min-w-0">
        <SelectCell
          value={merged.stock_status}
          options={STOCK_STATUS_OPTIONS}
          isDirty={isDirty('stock_status')}
...
function VirtualList({ items, selected, onRowClick, onToggleOne, dirtyMap, onCellCommit, onNavigate }) {
...
export default function TableBody({ loading, sorted, selected, onRowClick, onToggleOne, dirtyMap, onCellCommit, onNavigate }) {
  if (loading) {
    return (
