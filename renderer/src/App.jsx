...
export default function App() {
  ...
  const { syncing, syncLog, setSyncLog, runSync, stopSync } = useSync({
    settingsRef,
    setProductList,
    setPendingQueue,
    setOrderList,
  ...
  useEffect(() => {
    ...
    const unsub = window.electronAPI.onLicenseWarning((p) =>
      setLicenseWarning(p),
    );
    return () => unsub?.();
  }, []);
  ...
  useEffect(() => {
    ...
    if (
      window.electronAPI?.dbLoadProfile &&
  ...
  return (
    <div className="h-screen flex flex-col overflow-hidden bg-[linear-gradient(to_top,#f3e7e9_0%,#e3eeff_99%,#e3eeff_100%)] dark:bg-none dark:bg-[#111110]">
      ...
      {profileOpen && (
        <ProfileModal onClose={() => setProfileOpen(false)} license={license} />
      )}
    </div>
  );
}